...
app/
  ...
  atcoder/
    ...
    get_resister_id.py     登録者IDを返す (解除権限チェック用)
...
|---|---|---|
| atcoder_name | str | AtCoderユーザー名 |
| discord_user | Member | 紐付けるDiscordユーザー |
...
表示内容: 順位、パフォーマンス、レート変化、rated/unrated分類
...
毎日 1:05 JST に実行。前回処理したABC番号の次のコンテスト結果をAtCoder APIで確認し、レート確定済みであれば登録ユーザーのランキングを自動投稿する。投稿後は `prev_contest` を更新して二重投稿を防止する。
...
| resister_id | INTEGER | 登録操作を行ったユーザーのDiscord ID |
...
auto_abc_ranking
  ...
  -> Discordに投稿 (rated/unrated分類のランキング)
```
...
4. prev_contestが未設定の場合、APIから直近のABC番号を取得して初期化
5. ログ出力用チャンネルを設定
