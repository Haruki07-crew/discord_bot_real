...
app/
  ...
  atcoder/
    ...
    get_ac_streak.py                   昨日から遡って何日連続でACしているかを返す (今日は含めない)
...
表示内容: 各ユーザーの総AC数、昨日から遡って何日連続でACしているか (今日の分は含めない)

ソート: 連続AC日数(降順) -> 総AC数(降順)
...
1時間ごとに実行。前回処理したABC番号 (初期値: 454) の次のコンテスト結果をAtCoder APIで確認し、レート確定済みであれば登録ユーザーのランキングを自動投稿する。投稿後は `prev_contest` を更新して二重投稿を防止する。
